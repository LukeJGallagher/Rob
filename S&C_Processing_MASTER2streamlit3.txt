...
for idx, uploaded_file in enumerate(uploaded_files):
    ...
    jump = "DJ"  # Make sure this is defined before using it
    ...
    # Display information in the corresponding tab
    with tabs[idx]:
        st.header(f"Details for File {idx + 1}")
        st.write(f"**File Name:** {file_name}")
        st.write(f"**Athlete:** {athlete}")
        st.write(f"**Jump Type:** {jump}")  # 'jump' is now properly defined before this line
        st.write(f"**Leg:** {leg}")
        st.write(f"**Date of Trial:** {DATE}")

        # Create tabs for visualization and calculations
        chart_tab, calc_tab = st.tabs(["Chart", "Calculations"])
        
                 
            # Chart-related calculations
    with chart_tab:
                st.write("Chart visualization logic goes here")
            ...
            # Ensure calculation logic is inside the calc_tab context
with calc_tab:
                ...
                if 'F1z' in locals() and 'F2z' in locals() and len(F1z) >= 5000 and len(F2z) >= 5000:
                    ...
                    st.write("Error: F1z and/or F2z do not have enough data points.")
...
resP1, resP2 = np.array([]), np.array([])  # Use empty numpy arrays to avoid NameError
...
if 'F1z' in locals() and 'F2z' in locals() and len(F1z) >= 5000 and len(F2z) >= 5000:
    ...
    # Debugging Output
    st.write(f"Debug: Length of P1F = {len(P1F)}, Length of P2F = {len(P2F)}")
    ...
    if len(P1F) >= 1501 and len(P2F) >= 1501:
        ...
        # Debugging: Output the calculated body weights
        st.write(f"Debug: Calculated BW1 = {BW1}, BW2 = {BW2}")

        # Use BW from Plate 2 if Plate 1 data is unreliable
        if BW1 > 200 and BW2 > 200:
            BW = (BW1 + BW2) / 2  # Use the average of both plates if both are valid
        elif BW2 > 200:
            BW = BW2  # Fallback to Plate 2 if Plate 1 is not reliable
        else:
            st.write("Error: Body weight conditions not met.")
        ...
        st.write(f"Error: P1F and/or P2F do not have the minimum required data points (1501). P1F length: {len(P1F)}, P2F length: {len(P2F)}")
else:
    st.write("Error: F1z and/or F2z do not have enough data points.")

# Check if resP1 is not empty and has sufficient data before further calculations
if resP1.size > 0 and len(resP1) >= 501:
    maxresP1 = np.amax(resP1[0:501])
    st.write(f"Maximum residual for Plate 1 (first 501 points): {maxresP1}")
else:
    st.write(f"Error: resP1 is empty or does not have enough data points. resP1 size: {resP1.size}")
...
if resP1.size > 0 and len(resP1) >= 501:
    ...
    st.write("Error: resP1 is empty or does not have enough data points.")
    st.write("### Debug Information")
    st.write(f"First 10 values of F1z: {F1z[:10]}")
    st.write(f"First 10 values of P1F: {P1F[:10]}")
    st.write(f"First 10 values of F2z: {F2z[:10]}")
    st.write(f"First 10 values of P2F: {P2F[:10]}")
    ...
    # Additional debug statements to understand data patterns
st.write(f"Debug: Full length of F1z = {len(F1z)}, Full length of F2z = {len(F2z)}")
st.write(f"Debug: First 100 values of F1z: {F1z[:100]}")
st.write(f"Debug: First 100 values of F2z: {F2z[:100]}")
...
# Initialize ConF with a default value to avoid NameError if calculation fails
ConF = None

# Ensure the indices are valid before performing the calculation
if maxnegdisploc is not None and SoJ is not None and TO2 is not None:
    # Calculate the indices to check if they are valid
    start_index = maxnegdisploc - SoJ
    end_index = max(0, TO2 - SoJ)  # Ensure end_index is non-negative

    # Debugging output to verify the indices
    st.write(f"Debug: maxnegdisploc = {maxnegdisploc}, SoJ = {SoJ}, TO2 = {TO2}")
    st.write(f"Debug: Calculated Start index = {start_index}, End index = {end_index}, Length of FzJ = {len(FzJ)}")

    # Ensure the indices are within valid bounds
    if 0 <= start_index < len(FzJ) and start_index < end_index <= len(FzJ):
        # Extract the slice and verify its length
        data_slice = FzJ[start_index:end_index]
        st.write(f"Debug: Length of data slice = {len(data_slice)}")

        # Check if the slice is non-empty
        if len(data_slice) > 0:
            ConF = np.amax(data_slice)
            st.write(f"Maximum value in the data slice: {ConF}")
        else:
            st.write("Error: Data slice is empty, cannot calculate the maximum.")
    else:
        st.write("Error: Calculated indices are out of bounds or invalid.")
else:
    st.write("Error: One or more index variables (maxnegdisploc, SoJ, TO2) are not properly set.")

# Proceed with using ConF
if ConF is not None:
    # Use a tolerance to find a match in case of floating-point discrepancies
    tolerance = 1e-5  # Adjust the tolerance if needed
    ConFloc = np.where(np.abs(FzJ - ConF) <= tolerance)

    # Check if ConFloc is non-empty
    if len(ConFloc[0]) > 0:
        ConFloc = ConFloc[0][0] + SoJ + 1
        st.write(f"ConF location calculated: {ConFloc}")
    else:
        st.write("Error: ConF value not found in FzJ, even with tolerance.")
        # Gracefully handle the situation when no match is found
        ConFloc = None  # or set a default value if necessary
else:
    st.write("Error: ConF calculation failed, ConF is None.")

# Only proceed if ConFloc is successfully calculated
if ConFloc is not None:
    # Further processing with ConFloc
    st.write(f"Debug: ConFloc calculated successfully: {ConFloc}")
else:
    st.write("Warning: ConFloc was not calculated, skipping further operations.")

st.write(f"Debug: ConF value = {ConF}")
within_tolerance = FzJ[np.abs(FzJ - ConF) <= tolerance]
st.write(f"Values in FzJ within tolerance of ConF: {within_tolerance}")
