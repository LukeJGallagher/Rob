...
# =============================#       VALIDATION CHECK       # =============================
if len(P1F) >= 1501 and len(P2F) >= 1501:    # Perform calculations only if data points are sufficient
    res1P1 = np.sqrt(np.power(P1F - BW, 2))    resP1 = np.sqrt(np.power(P1F, 2))    resP2 = np.sqrt(np.power(P2F, 2))
else:    st.write('Error: Insufficient data points in `P1F` or `P2F`. Unable to perform calculations.')    resP1 = None    resP2 = None
