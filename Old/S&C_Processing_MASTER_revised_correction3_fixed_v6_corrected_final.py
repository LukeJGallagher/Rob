...
print(f'Error: The file {filepath} is empty. Skipping processing.')
continue
for filepath in filez:  # Loop through multiple files if more than one is selected
...
if os.path.getsize(filepath) == 0:
print(f'Error: The file {filepath} is empty. Skipping processing.')
raise ValueError('Empty file encountered during processing')
filepath = filez[0]  # Use a single file selection
...
# Safeguard for empty data before accessing elements or plotting
if data.empty:
print('Error: Attempted to access or plot data that is empty. Skipping operation.')
continue
...
# Prepare data for export
DJ = np.hstack((DATE, f'{leg} {BoxH}', BWkg))  # Add remaining data as needed
header2aDJ = ['Date Collected', 'Leg/BoxH', 'Body Mass (kg)']  # Add remaining headers as needed
